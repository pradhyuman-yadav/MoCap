...
#error "On an arduino Zero, BNO055's ADR pin must be high. Fix that, then delete this line."
    Adafruit_BNO055 ( int32_t sensorID = -1, uint8_t address = BNO055_ADDRESS_B, int32_t mux1port = -1, int32_t mux2port = -1);
#else
    Adafruit_BNO055 ( int32_t sensorID = -1, uint8_t address = BNO055_ADDRESS_A,  int32_t mux1port = -1, int32_t mux2port = -1);
#endif
