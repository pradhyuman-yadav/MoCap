...
/**************************************************************************/
/*!
    @brief  Sets the muxes to the correct state.
*/
/**************************************************************************/

bool Adafruit_BNO055::setupMux()
{
  //mux 2 has value seupt up mux1 to route to mux 2 and set mux 2 to port
  if(_mux2port != -1)
	{
	  //setup mux1 to route to mux2
	  Wire.beginTransmission(MUX1_ADDRESS);
	  #if ARDUINO >= 100
	    Wire.write(0x1 << MUX2_MUX1_PORT);
	  #else
	    Wire.send(0x1 << MUX2_MUX1_PORT);
	  #endif
	  Wire.endTransmission();

	  //setup mux2 to sensor's port
	  Wire.beginTransmission(MUX2_ADDRESS);
	  #if ARDUINO >= 100
	    Wire.write(0x1 << _mux2port);
	  #else
	    Wire.send(0x1 << _mux2port);
	  #endif
	  Wire.endTransmission();
	//mux 2 is -1 so we just route mux 1 to correct output.
	} else if (_mux1port != -1)
	{
	  //setup mux1 to sensor's port
	  Wire.beginTransmission(MUX1_ADDRESS);
	  #if ARDUINO >= 100
	    Wire.write(0x1 << _mux1port);
	  #else
	    Wire.send(0x1 << _mux1port);
	  #endif
	  Wire.endTransmission();
	}
  /* ToDo: Check for error! */
